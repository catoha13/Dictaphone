...
class AGAudioRecorder: NSObject {
    ...
    private var count = 0
    // надо разобраться как сохранить запись и передать ее в другой контроллер
//    var record = Record(name: self().filename, duration: 0, date: self().date)
